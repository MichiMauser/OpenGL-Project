...
uniform vec3 l_pos[3];
uniform vec3 l_color[3];
...
{		
	...
	float dist;
	if(toggle_p){
		dist = length(lightDir - lightPosWorld.xyz);
		attr = 1.0f / (cons + linear * dist + quadric * (dist * dist));
		}
	else{
		attr = 1.0f;
	}
	...
	ambient = ambientStrength * lightColor * attr;
	
	//compute diffuse light
	
	diffuse =  max(dot(normalEye, lightDirN), 0.0f) * lightColor * attr;
	
	//compute specular light
	vec3 reflection = reflect(-lightDirN, normalEye);
	float specCoeff = pow(max(dot(viewDirN, reflection), 0.0f), shininess);
	specular =  specularStrength * specCoeff * lightColor * attr;	
}
...
void main() 
{
	...
	vec4 finalColor;
	if(activ_tr){
		finalColor = vec4(color, a) * colorFromTexture;
	}else{
		finalColor = vec4(color, 1.0f) * colorFromTexture;
	}
